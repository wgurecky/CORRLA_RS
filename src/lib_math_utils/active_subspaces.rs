...
/// Active subspace identification methods
pub struct ActiveSsRsvd {
    grad_est: Box<dyn GradEst>,
    n_comps: usize,
}

/// Fitted Active subspace methods and data
pub struct FittedActiveSsRsvd {
    pub components_: Mat<f64>,
    pub singular_vals_: Mat<f64>,
    n_comps: usize,
}
...
impl FittedActiveSsRsvd {
    /// Create a fitted active subspace instance
    fn new(components: Mat<f64>, singular_vals: Mat<f64>, n_comps: usize) -> Self
    {
        Self {
            components_: components,
            singular_vals_: singular_vals,
            n_comps
        }
    }
    ...
    pub fn var_diag_evd_sensi(&self) -> Vec<f64> {
        let ndim = self.singular_vals_.as_ref().nrows();
        let mut evd_sensi: Vec<f64> = Vec::new();
        let grad_mat_reconstructed = self.components_.as_ref().transpose() *
            self.singular_vals_.as_ref() *
    ...
    pub fn components(&self) -> MatRef<f64> {
        self.components_.get(.., 0..self.n_comps)
    }

    /// Getter for singular_vals
    pub fn singular_vals(&self) -> MatRef<f64> {
        self.singular_vals_.get(.., 0..self.n_comps)
    }
}
...
impl ActiveSsRsvd {
    ...
    pub fn fit_svd(&self, x_mat: MatRef<f64>, n_iter: Option<usize>, n_oversamples: Option<usize>)
        -> FittedActiveSsRsvd
    {
        let k_features = x_mat.ncols();
        ...
        // return fitted active subspace
        FittedActiveSsRsvd::new(
            ur.to_owned(),
            mat_colvec_to_diag(sr.as_ref()).to_owned(),
            self.n_comps)
    }

    pub fn fit(&self, x_mat: MatRef<f64>)
        -> FittedActiveSsRsvd
    {
        let grad_mat = self.create_grad_mat(x_mat);
        ...
        // return fitted active subspace
        FittedActiveSsRsvd::new(
            sorted_components.to_owned(),
            sorted_singular_vals.to_owned(),
            self.n_comps)
    }
}
...
mod active_subspace_unit_tests {
    ...
    fn test_active_ss() {
        ...
        let tr_x_tst = fit_act_ss.transform(x_tst.as_ref());
        assert!(tr_x_tst.ncols() == n_comps);
        let tr_inv_x_tst = fit_act_ss.inv_transform(tr_x_tst.as_ref());
        ...
        let sens_coeffs = fit_act_ss.var_diag_evd_sensi();
